...
type SkuRtnJson struct {
	ProductList       []models.MinishopProduct   `json:"productList"`
	SpecificationList []models.SpecificationItem `json:"specificationList"`
}

type DetailRtnJson struct {
	SkuRtnJson
	Goods          models.MinishopGoods          `json:"info"`
	Galleries      []models.MinishopGoodsGallery `json:"gallery"`
...
type CategoryRtnJson struct {
	CurCategory     models.MinishopCategory   `json:"currentCategory"`
	ParentCategory  models.MinishopCategory   `json:"parentCategory"`
	BrotherCategory []models.MinishopCategory `json:"brotherCategory"`
}
...
type CommentInfo struct {
	...
	NickName string                          `json:"nick_name"`
	Avatar   string                          `json:"avatar"`
	PicList  []models.MinishopCommentPicture `json:"pic_list"`
}
...
func (this *GoodsController) Goods_Detail() {
	...
	var goodone models.MinishopGoods
	good := new(models.MinishopGoods)
	o.QueryTable(good).Filter("id", intGoodsId).One(&goodone)

	var galleries []models.MinishopGoodsGallery
	gallery := new(models.MinishopGoodsGallery)
	o.QueryTable(gallery).Filter("goods_id", intGoodsId).Limit(4).All(&galleries)

	qb, _ := orm.NewQueryBuilder("mysql")
	var attributes []Attribute
	qb.Select("ga.value", "a.name").
		From("minishop_goods_attribute ga").
		InnerJoin("minishop_attribute a").On("ga.attribute_id = a.id").
		Where("ga.goods_id =" + goodsId).OrderBy("ga.id").Asc()
	sql := qb.String()
	o.Raw(sql).QueryRows(&attributes)

	var issues []models.MinishopGoodsIssue
...
func (this *GoodsController) Goods_Category() {
	...
	o := orm.NewOrm()
	var curcategory models.MinishopCategory
	var parentcategory models.MinishopCategory
	var brothercategory []models.MinishopCategory
...
func (this *GoodsController) Goods_List() {
	...
	if keyword != "" {
		...
		searchhistory := models.MinishopSearchHistory{Keyword: keyword, UserId: utils.Int2String(getLoginUserId()),
...
func (this *GoodsController) Goods_Related() {
	...
	o := orm.NewOrm()
	relatedgoodstable := new(models.MinishopRelatedGoods)
	...
	if len(rgids) == 0 {
		var goodscategory models.MinishopGoods
