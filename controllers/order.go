...
func GetOrderPageData(rawData []models.MinishopOrder, page int, size int) utils.PageData {

	count := len(rawData)
	totalpages := (count + size - 1) / size
	var pagedata []models.MinishopOrder
...
type OrderListRtnJson struct {
	models.MinishopOrder
	GoodsList       []models.MinishopOrderGoods `json:"goodList"`
...
func (this *OrderController) Order_List() {
	...
	var orders []models.MinishopOrder
	...
	ordergoodstable := new(models.MinishopOrderGoods)
	var ordergoods []models.MinishopOrderGoods
	qsordergoods := o.QueryTable(ordergoodstable)
	for _, val := range firstpagedorders.Data.([]models.MinishopOrder) {
...
type OrderDetailRtnJson struct {
	...
	OrderGoods   []models.MinishopOrderGoods `json:"orderGoods"`
...
func (this *OrderController) Order_Detail() {
	...
	var orderinfo OrderInfo = OrderInfo{MinishopOrder: order}
...
func (this *OrderController) Order_Submit() {
	...
	orderinfo := models.MinishopOrder{
	...
	fmt.Println("going to insert the order")
	orderid, err := o.Insert(&orderinfo)
	if err != nil {
		fmt.Println("oder insert failed, err: ", err)
		this.Abort("订单提交失败")
	}
	orderinfo.Id = int(orderid)

	for _, item := range carts {
		ordergood := models.MinishopOrderGoods{
			OrderId:                   int(orderid),
