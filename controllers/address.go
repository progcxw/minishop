...
func (this *AddressController) Address_List() {
	...
	var addresses []models.MinishopAddress
...
func (this *AddressController) Address_Save() {
	...
	addressdata := models.MinishopAddress{
		Address:    address,
		CityId:     intcityid,
	...
	if isdefault {
		_, err := o.Raw("UPDATE minishop_address SET is_default = 0 where id <> ? and user_id = ?", intid, userid).Exec()
		if err == nil {
			//res.RowsAffected()
			//fmt.Println("mysql row affected nums: ", num)
		}
	}
	var addressinfo models.MinishopAddress
